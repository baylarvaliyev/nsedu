...
function CourseFaqEditRow({ item }: { item: CourseFaqItem }) {
  ...
  return (
    <div className="bg-[#f7f6f3] rounded-lg p-4 mb-3">
      ...
      {initialItems.length > 0 && (
        <div className="bg-white rounded-xl border border-[#e5e3dc] overflow-hidden">
          {initialItems.map((item) => (
            <div key={item.id} className="px-4 py-3 border-b border-[#f0eee8] last:border-0">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  >
                    {editingId === item.id ? <X size={15} /> : <Pencil size={15} />}
                  </button>
                  <button onClick={() => handleDelete(item.id)} className="text-red-600 hover:text-red-700" aria-label="Delete question">
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
